...
class ScenarioConfig(BaseModel):
    ...
    def render_system_prompt(self, context: dict[str, str] | None = None) -> str:
        ...
        return (
            ...
            "- Acknowledge what the caller said before steering the conversation forward; "
            "do not repeat the same question back-to-back.\n"
            ...
            "- Stay on the stated objective. Only ask questions that directly advance it. "
            "If the caller mentions something tangential (their cold, the weather, an unrelated "
            "complaint), acknowledge briefly with empathy and steer back to the goal. "
            "Do not pursue extra information the caller did not volunteer.\n"
            ...
            "- When the caller corrects themselves or switches a previously stated value "
            "(e.g. 'no wait, make it 5:30 PM instead'), immediately accept the new value as the "
            "ground truth, repeat it back crisply for confirmation, and discard the old one. "
